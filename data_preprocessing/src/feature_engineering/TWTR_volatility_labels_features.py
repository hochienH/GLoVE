...
DATA_PATH = os.path.join(ROOT_DIR, "data", "TWTR.parquet")
...
def build_alpha_input(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(cols) < 7:
        raise ValueError("OHLC.parquet 應至少包含 7 個欄位。")
...
def main() -> None:
    ...
    out_parquet = os.path.join(DATA_DIR, "TWTR_volatility_labels_features.parquet")
    out_csv = os.path.join(DATA_DIR, "TWTR_volatility_labels_features.csv")
