...
echo ""
echo "=========================================="
echo "STEP 1: LSTM baseline Optuna 搜尋"
echo "=========================================="

python src/LSTM/model_train_lstm_optuna_Base.py \
  --data $DATA_PATH \
  ...
  --log_dir $LOG_DIR \
  --output_model $MODEL_DIR/base/lstm.pth \
  --trial_csv $OUTPUT_DIR/optuna_lr_trials.csv \
  --seed 42 \
  --search_hidden_dim 16 32 64 \
  --search_num_layers 1 2 3 \
...
emit("BEST_HIDDEN_DIM", int(best["hidden_dim"]))
emit("BEST_NUM_LAYERS", int(best["num_layers"]))
...
echo "最佳參數："
echo "  lr          = $BEST_LR"
echo "  hidden_dim  = $BEST_HIDDEN_DIM"
echo "  num_layers  = $BEST_NUM_LAYERS"
...
cfg["best_lr"] = "$BEST_LR"
cfg["best_hidden_dim"] = "$BEST_HIDDEN_DIM"
cfg["best_num_layers"] = "$BEST_NUM_LAYERS"
...
echo ""
echo "=========================================="
echo "STEP 2: 掃描所有 λ，使用最佳超參數訓練 LSTM"
echo "=========================================="
...
run_train_tsmixer() {
    ...
    echo "       (lr=$BEST_LR, hidden_dim=$BEST_HIDDEN_DIM, num_layers=$BEST_NUM_LAYERS, dropout=$BEST_DROPOUT)"

    python src/LSTM/model_train_lstm.py \
        --data "$DATA_PATH" \
        --lambda "$lam" \
        --epochs $EPOCHS \
        --batch_size $BATCH_SIZE \
...
echo ""
echo "=========================================="
echo "STEP 3: LSTM λ sweep — Prediction + Visualization"
echo "=========================================="
...
run_predict_vis() {
    ...
    CMD_PREDICT=(
        python src/LSTM/predict_lstm.py
        --data "$DATA_PATH"
